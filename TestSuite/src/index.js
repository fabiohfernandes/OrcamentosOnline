...
const ClaudeIntegration = require('./claude/claudeIntegration');
...
class TesterMain {
    constructor() {
        ...
        this.claude = new ClaudeIntegration(this.activityLogger, this.db);
    ...
    async startTestSession(config) {
        ...
        // Log session start and signal Claude
        await this.activityLogger.logSessionStart(sessionId, config);
        await this.activityLogger.signalSessionStarted(sessionId, config);
        ...
        // Signal feature discovery completion
        if (coverage.features && coverage.features.length > 0) {
            await this.activityLogger.signalFeatureDiscoveryCompleted(sessionId, coverage);
        }
    ...
    async initialize() {
        try {
            ...
            // Initialize activity logger with log directory
            const logDirectory = path.join(process.cwd(), '../OrçamentosOnlineTestSuite/logs');
            await this.activityLogger.initialize(logDirectory);
            console.log(chalk.green('✅ Activity logger initialized'));
            ...
            await this.claude.initialize(logDirectory);

            // Start Claude monitoring
            await this.claude.startMonitoring();
