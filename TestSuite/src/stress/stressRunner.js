...
class StressRunner {
    ...
    async executeNavigationTest(user, flow) {
        ...
        const link = this.getLocator(user.page, flow.selector);
    ...
    async executeInteractionTest(user, flow) {
        ...
        const element = this.getLocator(user.page, flow.selector);
    ...
    // Helper method to handle different selector types (CSS and text-based)
    getLocator(page, selector) {
        if (selector.startsWith('text=')) {
            // Handle text-based selectors for Playwright
            const text = selector.substring(5); // Remove 'text=' prefix
