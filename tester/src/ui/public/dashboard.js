...
class TesterDashboard {
    ...
    bindEventListeners() {
        // Control buttons
        document.getElementById('startBtn').addEventListener('click', () => this.startTesting());
    ...
    async startTesting() {
        try {
            const config = {
                ...
                virtualUsers: parseInt(document.getElementById('virtualUsers').value) || 5,
                duration: parseInt(document.getElementById('duration').value) || 10,
                claudeAutonomous: document.getElementById('claudeAutonomous').checked
            };
            ...
            if (result.success) {
                this.addLogEntry({
                    type: 'success',
                    message: `Testing session started with ${config.virtualUsers} virtual users`,
                    timestamp: new Date()
                });
            } else {
                throw new Error(result.error || 'Failed to start testing');
            }
    ...
    async stopTesting() {
        try {
            ...
            this.addLogEntry({
                type: 'error',
                message: `Failed to stop testing: ${error.message}`,
                timestamp: new Date()
            });
        }
    }
